  ...
  useEffect(() => {
    ...
    const handleUnhandledRejection = (event) => {
      ...
      if (
        ...
        event.reason.includes("window.destroy not allowed")
      ) {
        console.log(
          "🔧 Intercepted window.destroy permission error - this is expected",
        );
  ...
  useEffect(() => {
    ...
    currentWindow
      .onCloseRequested(async (event) => {
        console.log(
          "🔧 Window close requested, isQuitting:",
        ...
        if (
          ...
          console.log(
            "🛑 Window close prevented - showing unsaved changes dialog",
          );
            ...
            if (currentTempIdRef.current) {
              try {
                await invoke("delete_temp_file", {
                  tempId: currentTempIdRef.current,
                });
  ...
  useEffect(() => {
    ...
    const updateDisplayContent = async () => {
      if (currentFile && fileContent) {
        const converted = await convertMarkdownImagePaths(
          fileContent,
          currentFile,
        );
  ...
  const loadTempFilesOnStartup = async () => {
    try {
      ...
      if (tempFiles && tempFiles.length > 0) {
        console.log(
          `🔄 Found ${tempFiles.length} unsaved file(s) from previous session`,
        );
  ...
  const handleExportToPdf = async () => {
    try {
      ...
      if (!previewElement) {
        ...
        alert(
          "Could not find preview. Please make sure you are in Preview or Live mode.",
        );
        return;
      }
...
const openRecentItem = async (item) => {
  ...
  const handleThemeChange = (newTheme) => {
    ...
    if (omakaseSyncEnabled && omakaseAvailable) {
      toast.error(
        "Omakase sync is enabled. Please disable it in Settings to manually change themes.",
        {
          duration: 4000,
          icon: "🎨",
        },
  ...
  return (
    <div className={`app`}>
      ...
      <div className="app-body">
        {isSidebarVisible && !isFullscreen && (
          <Sidebar
            ...
            onRefreshFiles={() => {
              if (currentFolder) {
                invoke("get_folder_files", { folderPath: currentFolder }).then(
                  setFiles,
                );
              }
            }}
      ...
      <RecoveryDialog
        ...
        fileName={
          recoveryDialog.tempFile
            ? getSmartFileName(recoveryDialog.tempFile.content)
            : "Untitled"
        }
        preview={
          recoveryDialog.tempFile
